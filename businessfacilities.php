<?php
    ...
    class BusinessFacilities {
        private static $table_name = "businessfacilities";
        protected static $db_fields = array('id', 'verify_string', 'business_string', 'facility_string', 'created', 'last_updated');
        public $id;
        public $verify_string;
        public $business_string;
        public $facility_string;
        public $created;
        public $last_updated;
        ...
        private function check_errors(){
            if(empty($this->business_string)){
                $this->errors[] = "Business must be povided";
            }
            if(empty($this->facility_string)){
                $this->errors[] = "Facility must be provided";
            }
        }
        ...
        public function insert(){
            ...
            if(empty($this->errors)){
                ...
                if($this->save()){
                    ...
                    $this->errors[] = "The Facility was not added";
                    return false;
                }
            } else {
                return false;
		...
		public static function find_by_business_facility($business="", $facility=""){
		    global $database;
		    $result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE business_string='{$business}' AND facility_string='{$facility}' LIMIT 1");
		    return !empty($result_array) ? array_shift($result_array) : false;
		}
