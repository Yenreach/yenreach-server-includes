<?php
    ...
    class BusinessReviews {
        private static $table_name = "businessreviews";
        protected static $db_fields = array('id', 'verify_string', 'user_string', 'business_string', 'review', 'star', 'created', 'last_updated');
        public $id;
        ...
        private function check_errors(){
            if(empty($this->user_string)){
                $this->errors[] = "Only a User can drop a review";
            }
            if(empty($this->business_string)){
                $this->errors[] = "Only a Business can be reviewed";
            }
            if(empty($this->review)){
                $this->errors[] = "You cannot submit an empty review";
            }
        }
        ...
        public function insert(){
            ...
            if(empty($this->errors)){
                ...
                if($this->save()){
                    ...
                    $this->errors[] = "Business Review was not saved";
                    return false;
                }
            } else {
                return false;
		...
		public static function find_by_user_business($user="", $business=""){
		    $result_array = self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE user_string='{$user}'AND business_string='{$business}' LIMIT 1");
		    return !empty($result_array) ? array_shift($result_array) : false;
		}
